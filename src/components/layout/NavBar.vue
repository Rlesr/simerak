    ...
    >
        ...
        <div class="lg:hidden flex items-center z-50">
            <button @click="toggleMenu" class="text-white focus:outline-none">
                <span :class="{ 'transform rotate-45 translate-y-1.5': menuOpen }" class="block w-6 h-0.5 bg-white mb-1 transition-all duration-300"></span>
                <span :class="{ 'opacity-0': menuOpen }" class="block w-6 h-0.5 bg-white mb-1 transition-all duration-300"></span>
                <span :class="{ 'transform -rotate-45 -translate-y-1.5': menuOpen }" class="block w-6 h-0.5 bg-white transition-all duration-300"></span>
            </button>
        </div>
        ...
        >
            <li v-for="(link, index) in navLinks" :key="index" class="w-full lg:w-auto relative group">
                <a
                    :href="link.href"
                    @click="
                        setActiveLink(index);
                ...
                >
                    {{ link.name }}
                    <span v-if="activeLink === index" class="absolute bottom-0 left-1/2 transform -translate-x-1/2 h-1 w-8 bg-[#F57900] rounded-full"></span>
                </a>
            </li>
            ...
            <li class="relative w-full lg:w-auto group">
                ...
                >
                    Master
                    <span v-if="dropdownOpen" class="absolute bottom-0 left-1/2 transform -translate-x-1/2 h-1 w-8 bg-[#F57900] rounded-full"></span>
                </button>
                <ul v-if="dropdownOpen" class="lg:absolute lg:left-0 lg:mt-2 w-full lg:w-[250px] bg-white text-sm text-black shadow-lg rounded-lg lg:z-50">
                    <li v-for="(item, index) in masterItems" :key="index" class="hover:bg-gray-200">
                        <a
                            href="#"
                            @click="
                                dropdownOpen = false;
